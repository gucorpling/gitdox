...
if INTP_VER < (3, 0):
    raise RuntimeError("Python v.3.0 or later needed")
...
class Section(dict):
    ...
    def decode(self, encoding):
        ...
        ...             import sys
        ...             print(type(entry), file=sys.stderr)
        ...             raise AssertionError('decode failed.')
        ...         if isinstance(val[entry], dict):
        ...             testuni(val[entry])
        ...         elif not isinstance(val[entry], str):
        ...             raise AssertionError('decode failed.')
        >>> testuni(m)
        >>> m.encode('ascii')
        >>> a == m
        ...
        def decode(section, key, encoding=encoding):
            ...
            if isinstance(val, (list, tuple)):
                newval = []
                for entry in val:
                    newval.append(entry.decode(encoding) if hasattr(entry, 'decode') else entry)
            elif isinstance(val, dict):
                newval = val
            else:
                newval = val.decode(encoding) if hasattr(val, 'decode') else val
            newkey = key.decode(encoding) if hasattr(key, 'decode') else key
            section.rename(key, newkey)
    ...
    def encode(self, encoding):
        ...
        def encode(section, key, encoding=encoding):
            ...
            if isinstance(val, (list, tuple)):
                newval = []
                for entry in val:
                    newval.append(entry.encode(encoding) if hasattr(entry, 'encode') else entry)
            elif isinstance(val, dict):
                newval = val
            else:
                newval = val.encode(encoding) if hasattr(val, 'encode') else val
            newkey = key.encode(encoding) if hasattr(key, 'encode') else key
            section.rename(key, newkey)
            section[newkey] = newval
...
class ConfigObj(Section):
    ...
    def __init__(self, infile=None, options=None, **kwargs):
        ...
        if isinstance(infile, StringTypes):
            ...
            raise TypeError('infile must be a filename, StringIO instance, or a file as a list.')
        #
        # strip trailing '\n' from lines
        infile = [line.rstrip('\n') for line in infile]
        #
    ...
    def validate(self, validator, preserve_errors=False, section=None):
        ...
        ... except ImportError:
        ...     import sys
        ...     print('Cannot import the Validator object, skipping the related tests', file=sys.stderr)
        ... else:
        ...     config = '''
        ...     test1=40
        ...     test2=hello
