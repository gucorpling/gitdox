  ...
  return (
    ...
    >
      ...
      <main className="p-6 flex-1" style={Object.keys(mainStyle).length > 0 ? mainStyle : undefined}>
        ...
        {token && currentView.startsWith('document:') && <DocumentEditor apiCall={apiCall} docId={currentView.split(':')[1]} onCorpusChange={setActiveDocCorpus} user={user} token={token} projectName={projectName} mutationTools={toolConfig} spannotatorConfig={appConfig?.entities || {}} dendroidConfig={appConfig?.dendroid || {}} spreadsheetConfig={appConfig?.spreadsheet || {}} editorOptions={editorOptions} editorFonts={{ ui: appConfig?.ui, xml: appConfig?.xml, entities: appConfig?.entities, spreadsheet: appConfig?.spreadsheet }} spreadsheetColumnOrderConfig={spreadsheetColumnOrder} xmlTagCompletion={xmlTagCompletion} statusCategories={statusCategories} isMainDark={isMainDark} isAppConfigLoaded={isAppConfigLoaded} />}
